class Solution:
    ...
    def generate_moves(
        self, x0: int, y0: int, dirs: List[Tuple[int, int]]
    ...
    def is_valid(
        self,
        move1: Tuple[int, int, int, int, int],
        move2: Tuple[int, int, int, int, int],
    ) -> bool:
    ...
    def countCombinations(self, pieces: List[str], positions: List[List[int]]) -> int:
        ...
        piece_dirs = {"rook": rook_dirs, "bishop": bishop_dirs, "queen": rook_dirs + bishop_dirs}
        # 预处理所有合法移动
        all_moves = [
            self.generate_moves(x, y, piece_dirs[piece])
